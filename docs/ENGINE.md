# Your Move — the engine, V1A

An experiential simulation built to the Adaptive Worlds design rules. It is entertainment
...
| 21 | Play Interface | `app/` + `components/yourmove/PlayApp.tsx` |
...
It is a **standalone implementation** that reads this engine's spine. The leadership
product and this one share the vocabulary, not the code, exactly as the convergence brief
specifies: one simulation core, two applications, two interpretation layers. Nothing in
`lib/aw` imports the lens, and the lens imports nothing that can write.
