...
Your Move uses **its own Supabase project**. It shares no table, no schema, and no
credential with The Signal, and nothing in `lib/aw` or `lib/yourmove` ever holds The
Signal's keys.
...
2. **Apply the migration.** Supabase Dashboard → **SQL Editor** → New query → paste the
   entire contents of `supabase-yourmove/migrations/0001_yourmove.sql` → **Run**. Or, from
   a machine with `psql` and the project's connection string:

   ```bash
   psql "$YOURMOVE_DB_URL" -f supabase-yourmove/migrations/0001_yourmove.sql
   ```
...
`.env.yourmove.example` has all of this in copy-paste form.
...
read and write goes through the server, exactly as The Signal does it.
...
Add numbered files to `supabase-yourmove/migrations/`. Do not edit `0001` after it has been
